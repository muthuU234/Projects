...
const Accepted=()=>{
    ...
    const fetching=async ()=>{
            ...
            try{
            const api=await axios.get(`http://localhost:8080/client/accepted_order/${val}`);
...
`}</style>
      ...
      {!isbool ? (
        ...
        <div>
            <center className="cntrtag">Accepted Orders</center>
          ...
          <tbody>
            {res.map((e, index) => (
              <tr key={index}>
                ...
                <td data-label="Rejected Time">{e.cname}</td>
                <td>{e.bid}</td>
              </tr>
            ))}
          </tbody>
